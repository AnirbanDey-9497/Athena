...
const SettingsPage = () => {
  ...
  useEffect(() => {
    const fetchFirstViewSetting = async () => {
      try {
        const response = await getFirstView()
        if (response?.status === 200) {
          setFirstView(response.data)
        }
      } catch (error) {
    ...
    fetchFirstViewSetting()
  }, [])

  const handleFirstViewToggle = async (checked: boolean) => {
    if (isLoading) return
  ...
  return (
    <div>
      ...
      <Label className="flex items-center gap-x-3 mt-4 text-md">
        ...
        <Switch
          checked={firstView}
          onCheckedChange={handleFirstViewToggle}
          disabled={isLoading}
        />
      </Label>
    </div>
